...
async def create_processed_agent_data(data: List[ProcessedAgentData]):
    ...
    async with async_session() as session:
        try:
            for item in data:
                # Створення рядка для вставки в БД
                ...
                # Виконання запиту на додавання рядка до БД
                await session.execute(db_item)
            await session.commit() # Застосування всіх змін до БД
...
async def list_processed_agent_data():
    ...
    async with async_session() as session:
        try:
            ...
            # Ініціалізація порожнього списку для збереження оброблених даних
            processed_data = []
            for row in data:
                # Створення об'єкту ProcessedAgentDataInDB з кожного рядка та додавання його до списку
...
async def read_processed_agent_data(processed_agent_data_id: int):
    ...
    async with async_session() as session:
        try:
            # Створення запиту до БД для отримання даних за Id
            statement = select(processed_agent_data).where(processed_agent_data.c.id == processed_agent_data_id)
            result = await session.execute(statement)
            data = result.fetchone()
            # Перевірка, чи знайдені дані за вказаним Id
            if data is None:
                raise HTTPException(status_code=404, detail="Data not found")
            # Створення об'єкту ProcessedAgentDataInDB з рядка результату
...
async def update_processed_agent_data(processed_agent_data_id: int, data: ProcessedAgentData):
    ...
    async with async_session() as session:
        try:
            # Отримання рядка за його Id
            statement = select(processed_agent_data).where(processed_agent_data.c.id == processed_agent_data_id)
            result = await session.execute(statement)
            db_row = result.fetchone()

            # Перевірка чи існує рядок з вказаним Id
            if db_row is None:
                raise HTTPException(status_code=404, detail=f"Row with ID {processed_agent_data_id} not found")

            # Оновлення значень рядка
            db_row = db_row._asdict()  # Конвертуємо рядок у словник
            db_row.update({
...
async def delete_processed_agent_data(processed_agent_data_id: int):
    ...
    async with async_session() as session:
        try:
            # Створення запиту до БД для видалення рядка за його Id
